...
/***/ function(module, exports, __webpack_require__) {
	...
	var DateTime = __webpack_require__(157);
	var Arc = __webpack_require__(158);
	var SunCalc = __webpack_require__(176);
	var Zenometer = __webpack_require__(177);

	var _require = __webpack_require__(178);

	var cartesianToAngle = _require.cartesianToAngle;

	function Circle(center, radius) {
	  return {
	    center: center,
	    radius: radius,
	    getPointAtAngle: function getPointAtAngle(angle) {
	      return {
	...
	var ClockHours = React.createClass({
	  ...
	  render: function render() {
	    ...
	    var numberCircle = new Circle(center, radius - 15);

	    var times = [];
	    for (var i = 0; i < 24; i++) {
	      var angle = i / 24 * 2 * Math.PI - Math.PI / 2;

	      var _numberCircle$getPointAtAngle = numberCircle.getPointAtAngle(angle);

	      var x = _numberCircle$getPointAtAngle.x;
	      var y = _numberCircle$getPointAtAngle.y;

	      var _getPointAtAngle = new Circle(center, radius - 5).getPointAtAngle(angle);

	      var innerX = _getPointAtAngle.x;
	      var innerY = _getPointAtAngle.y;

	      var _circle$getPointAtAngle = circle.getPointAtAngle(angle);

	      var outerX = _circle$getPointAtAngle.x;
	      var outerY = _circle$getPointAtAngle.y;

	      times.push(React.createElement(
	        'g',
	        null,
	        React.createElement('path', { className: 'tick', d: 'M ' + innerX + ' ' + innerY + ' L ' + outerX + ' ' + outerY }),
	        React.createElement(
	          'text',
	          { className: 'label', x: x, y: y, transform: 'rotate(' + angle / Math.PI * 180 + ' ' + x + ' ' + y + ')' },
	          i % 12 || 12
	        )
	      ));
	    }
	    ...
	    return React.createElement(
	      'g',
	      { className: 'hours' },
	      React.createElement('circle', { cx: center.x, cy: center.y, r: radius }),
	      times,
	      React.createElement(
	        'text',
	        { className: 'current', x: center.x, y: center.y + 15 },
	        time.toString()
	      )
	    );
	  }
	...
	var ClockHand = React.createClass({
	  displayName: 'ClockHand',

	  render: function render() {
	    var _props2 = this.props;
	    var circle = _props2.circle;
	    var date = _props2.date;
	    var center = circle.center;

	    var angle = date.toAngle();

	    var _circle$getPointAtAngle2 = circle.getPointAtAngle(angle);

	    var handX = _circle$getPointAtAngle2.x;
	    var handY = _circle$getPointAtAngle2.y;

	    return React.createElement('path', { className: 'clock-hand', d: 'M ' + center.x + ' ' + center.y + ' L ' + handX + ' ' + handY });
	  }
	});

	var SunClock = React.createClass({
	  displayName: 'SunClock',
	  ...
	  render: function render() {
	    ...
	    var times = SunCalc.getTimes(time.date, 37, -122);
	    var civilTwilightStart = new DateTime(times.sunset);
	    var nauticalTwilightStart = new DateTime(times.dusk);
	    var astronomicalTwilightStart = new DateTime(times.nauticalDusk);
	    var night = new DateTime(times.night);
	    var astronomicalDawnStart = new DateTime(times.nightEnd);
	    var nauticalDawnStart = new DateTime(times.nauticalDawn);
	    var civilDawnStart = new DateTime(times.dawn);
	    var dawn = new DateTime(times.sunrise);
	    ...
	    var arcs = ranges.map(function (range) {
	      return React.createElement(Arc, { className: 'arc ' + range['class'],
	        circle: circle,
	...
	var MoonClock = React.createClass({
	  displayName: 'MoonClock',

	  render: function render() {
	    var _props4 = this.props;
	    var time = _props4.time;
	    var circle = _props4.circle;

	    var times = SunCalc.getMoonTimes(time.date, 37, -122);
	    var aboveHorizonStart = new DateTime(times.rise);
	    var belowHorizonStart = new DateTime(times.set);
	    ...
	    return React.createElement(
	      ...
	      React.createElement('circle', { className: 'above-horizon', cx: circle.center.x, cy: circle.center.y, r: circle.radius }),
	      arcs
	    );
	  }
	});
	...
	var Clock = React.createClass({
	  ...
	  onClick: function onClick(e) {
	    ...
	    var xWithinElement = e.clientX - left;
	    var yWithinElement = e.clientY - top;
	    var xAroundCircle = xWithinElement - 256;
	    var yAroundCircle = 256 - yWithinElement;
	    this.props.handleTimePick(cartesianToAngle(xAroundCircle, yAroundCircle));
	  },
	  render: function render() {
	    var time = this.props.time;

	    var center = { x: 256, y: 256 };
	    var radius = 256;
	    ...
	    return React.createElement(
	      ...
	      { xmlns: 'http://www.w3.org/svg/2000',
	        ...
	        width: width,
	        height: height },
	      React.createElement('circle', { id: 'clock-path', cx: center.x, cy: center.y, r: radius }),
	      React.createElement(SunClock, { circle: circle, time: time }),
	      React.createElement(MoonClock, { circle: new Circle(center, radius / 2), time: time }),
	      React.createElement(ClockHand, { circle: circle, date: time }),
	      React.createElement(ClockHours, { circle: new Circle(center, radius / 2 - 20), time: time }),
	      React.createElement('rect', { fill: 'white', opacity: '0', x: 0, y: 0, width: width, height: height, onClick: this.onClick })
	    );
	  }
	});
	...
	var App = React.createClass({
	  ...
	  updateTimeFromClock: function updateTimeFromClock(angle) {
	    this.setState({
	  ...
	  render: function render() {
	    ...
	    return React.createElement(
	      'div',
	      null,
	      React.createElement(Clock, { time: date, handleTimePick: this.updateTimeFromClock }),
	      React.createElement(Zenometer, { time: date })
	    );
	  }
	});
...
/***/ function(module, exports) {
	...
	function timeToAngle(time) {
	  var secondsPastToday = time.getHours() * 60 * 60 + time.getMinutes() * 60 + time.getSeconds();
	  return secondsPastToday / secondsInDay * 2 * Math.PI - Math.PI / 2;
	}

	function secondsToHMS(secondsInDay) {
	  var hours = Math.floor(secondsInDay / (60 * 60));
	  var minutes = Math.floor((secondsInDay - 60 * 60 * hours) / 60);
	...
	function dateToTimeString(date) {
	  var match = /(\d{2}:\d{2}:\d{2})/.exec(date.toTimeString());
	  return match[1];
	}
	...
	function timeInputValueToDate(timeString) {
	  ...
	  var match = /(\d{2}):(\d{2})/.exec(timeString);
	...
	DateTime.fromAngle = function (angle) {
	  var time = secondsToHMS(angle / (2 * Math.PI) * secondsInDay);
	  var date = new Date();
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _objectWithoutProperties = __webpack_require__(159)["default"];
	...
	function describeArc(x, y, radius, startAngle, endAngle) {
	  ...
	  var d = ["M", x, y, "L", start.x, start.y, "A", radius, radius, 0, arcSweep, 0, end.x, end.y, "L", x, y].join(" ");
	...
	var Arc = React.createClass({
	  ...
	  render: function render() {
	    ...
	    var others = _objectWithoutProperties(_props, ["circle", "startTime", "endTime"]);

	    var center = circle.center;
	    var radius = circle.radius;

	    var startAngle = startTime.toAngle();
	    var endAngle = endTime.toAngle();

	    return React.createElement("path", _extends({ d: "" + describeArc(center.x, center.y, radius, startAngle, endAngle) }, others));
	  }
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Range = React.createClass({
	  ...
	  render: function render() {
	    ...
	    return React.createElement('path', { className: 'pin', d: 'M ' + width + ' ' + max + ' L ' + 1 + ' ' + max + ' L ' + 1 + ' ' + min + ' L ' + width + ' ' + min });
	  }
	});

	var Zenometer = React.createClass({
	  displayName: 'Zenometer',

	  render: function render() {
	    var time = this.props.time;

	    var sunPosition = SunCalc.getPosition(time.date, 37, -122);

	    var width = 32;
	    var height = 512;

	    var radianAltitudeScale = new Scale(-Math.PI / 2, Math.PI / 2, 512);

	    var pinPosition = radianAltitudeScale.at(sunPosition.altitude);
	    var times = SunCalc.getTimes(time.date, 37, -122);
	    var maxPosition = radianAltitudeScale.at(SunCalc.getPosition(times.solarNoon, 37, -122).altitude);
	    var minPosition = radianAltitudeScale.at(SunCalc.getPosition(times.nadir, 37, -122).altitude);
	    ...
	    return React.createElement(
	      ...
	      { xmlns: 'http://www.w3.org/svg/2000',
	        ...
	        height: height,
	        className: 'zenometer' },
	      React.createElement('rect', { className: 'area above-horizon', x: 0, y: 0, width: width, height: height / 2 }),
	      React.createElement('rect', { className: 'area below-horizon', x: 0, y: height / 2, width: width, height: height / 2 }),
	      React.createElement('rect', { className: 'area civil-twilight', x: 0, y: height / 2, width: width, height: 6 / 180 * height }),
	      React.createElement('rect', { className: 'area nautical-twilight', x: 0, y: height / 2 + 6 / 180 * height, width: width, height: 6 / 180 * height }),
	      React.createElement('rect', { className: 'area astronomical-twilight', x: 0, y: height / 2 + 12 / 180 * height, width: width, height: 6 / 180 * height }),
	      React.createElement('path', { className: 'pin', d: 'M ' + 10 + ' ' + pinPosition + ' L ' + width + ' ' + pinPosition }),
	      React.createElement(Range, { min: minPosition, max: maxPosition })
	    );
	  }
	});

	exports['default'] = Zenometer;
	module.exports = exports['default'];
...
/***/ function(module, exports) {
	...
	exports.__esModule = true;
	exports.cartesianToAngle = cartesianToAngle;

	function cartesianToAngle(xOrig, yOrig) {
	  var y = xOrig;
	  var x = yOrig;
